...
public class Aminoacids {
   ...
   * @param aSequence :
   *          pepitide sequence
   * @return getMonoPeptideMass peptide mass (add extra 18.0 to monoisotopic residues
   *         mass)
   */
  ...
  /**
   * The method used in Construction should be set to private method
   * to avoid surprised error caused by polymorphism.
   */
  ...
  private void initAminoacidZ() {
    totalAminoacids['Z'].reset(
        'Z',
        "Glx",
        (totalAminoacids['Q'].getMonoMass() + totalAminoacids['E'].getMonoMass()) / 2,
        (totalAminoacids['Q'].getAverageMass() + totalAminoacids['E'].getAverageMass()) / 2,
        (totalAminoacids['Q'].getHydropathicity() + totalAminoacids['E'].getHydropathicity()) / 2,
        "Glutamine or Glutamic acid",
        true);
  }

  private void initAminoacidB() {
    totalAminoacids['B'].reset(
        'B',
        "Asx",
        (totalAminoacids['D'].getMonoMass() + totalAminoacids['N'].getMonoMass()) / 2,
        (totalAminoacids['D'].getAverageMass() + totalAminoacids['N'].getAverageMass()) / 2,
        (totalAminoacids['D'].getHydropathicity() + totalAminoacids['N'].getHydropathicity()) / 2,
  ...
  private void initAminoacidX() {
    ...
    totalAminoacids['X'].reset(
        'X',
        "Xaa",
        averageMonoMass,
        averageAverageMass,
        averageHydrophthicity,
        "Any amino acid",
        true);
  }
