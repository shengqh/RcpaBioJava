 ...
 * but relies on a navigation image located in the upper left corner of the
 * panel. The navigation image is a small replica of the image displayed in the
 * panel. When you click on any point of the navigation image that part of the
 * image is displayed in the panel, centered. The navigation image can also be
 ...
 * </pre>
 * 
 * and use <code>getImageOrigin()</code> and <code>setImageOrigin()</code>
 * to move the image around the panel.
 * </p>
 * <h3>Rendering</h3>
 * <p>
 * <code>NavigableImagePanel</code> uses the Nearest Neighbor interpolation
 * for image rendering (default in Java). When the scaled image becomes larger
 * than the original image, the Bilinear interpolation is applied, but only to
 * the part of the image which is displayed in the panel. This interpolation
 * change threshold can be controlled by adjusting the value of
...
public class NavigableImagePanel extends JPanel {
	...
	private void drawZoomAreaOutline(Graphics g) {
		...
		int height = getHeight() * getScreenNavImageHeight()
